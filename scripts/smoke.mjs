#!/usr/bin/env node
// AgentGate plugin — local structure validation
...
if (!skillText.includes("name: agent-gate")) fail("SKILL.md missing frontmatter name");
...
if (manifest.name !== "agent-gate") fail("plugin.json name mismatch");
...
if (cursorManifest.name !== "agent-gate") fail("cursor plugin.json name mismatch");
